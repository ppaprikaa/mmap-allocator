...
class ConstMemView {
	public:
		...
		const size_t Pages() const noexcept {
...
class MutableMemView {
	public:
		MutableMemView(char* start, size_t size) : initial_(start), start_(start), size_(size) {}
		MutableMemView() : MutableMemView(nullptr, 0) {}

		static size_t PageSize();

		static MutableMemView Empty() {
			return MutableMemView();
		}

		char* Initial() const noexcept {
			return initial_;
		}

		char* Start() const noexcept {
			return start_;
		}
